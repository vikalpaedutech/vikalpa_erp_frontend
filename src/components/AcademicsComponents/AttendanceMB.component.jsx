...
const AttendanceMB = ({assignedDistricts, assignedBlocks, assignedSchools}) => {
    ...
    // New: Track isDressGiven toggles for each student
    const [isDressGiven, setIsDressGiven] = useState({});
...
console.log(allSchoolIds);
  ...
  ["9", "10"].every(id => userData.userAccess.classId.includes(id))){
        ...
        if (true) {
            try {
                ...
                response.data.forEach((attendance) => {
                    ...
                    initialDressAmountSubmitted[attendance.studentSrn] = attendance.studentDetails.dressAmountSubmitted || false;
                    initialIsDressGiven[attendance.studentSrn] = attendance.studentDetails.isDressGiven || false;
                });
                setAttendanceState(initialAttendanceState);
                setDressSizes(initialDressSizes);
                setDressAmountSubmitted(initialDressAmountSubmitted);
                setIsDressGiven(initialIsDressGiven);
            } catch (error) {
                console.log("Error fetching attendance data");
                setAttendanceData([])
            }
    ...
    // New: Function to handle Dress Given toggle (uses same API family patchStudentBySrn)
    const handleDressGivenToggle = async (studentSrn, currentValue) => {
        const newValue = !currentValue;
        setIsDressGiven((prev) => ({
            ...prev,
            [studentSrn]: newValue
        }));

        try {
            await patchStudentBySrn(studentSrn, { isDressGiven: newValue });
        } catch (error) {
            console.error("Error updating isDressGiven toggle", error.message);
        }
    };
                        ...
                        {attendanceData.length > 0 ? (
                            attendanceData.map((attendance, index) => (
                                <tr key={attendance._id}>
                                    ...
                                    {/* NEW: Dress Given toggle cell (right after #) */}
                                    <td>
                                      <div className="toggle-container">
                                        <div
                                          className={`toggle-button ${isDressGiven[attendance.studentSrn] ? "on" : "off"}`}
                                          onClick={() => handleDressGivenToggle(attendance.studentSrn, isDressGiven[attendance.studentSrn])}
                                        >
                                          <div className={`circle ${isDressGiven[attendance.studentSrn] ? "move-right" : "move-left"}`}></div>
                                          <span className={`toggle-text ${isDressGiven[attendance.studentSrn] ? "on-text" : "off-text"}`}>
                                            {isDressGiven[attendance.studentSrn] ? "Yes" : "No"}
                                          </span>
                                        </div>
                                      </div>
                                    </td>
