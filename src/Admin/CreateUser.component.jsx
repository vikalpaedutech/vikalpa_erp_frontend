...
export const CreateUser = () => {
  ...
  const [modules, setModules] = useState([]);
  const [region, setRegion] = useState([]);
  // ✅ Changed from classIds to batches
  ...
  // ✅ Updated to handle batches instead of classes
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const accessReq = {
        ...
        modules,
        region,
        // ✅ Send batch instead of classId
        batch: batches,
      };

      console.log("Access Request:", accessReq);
      await setUserAccess(accessReq);
  ...
  const handleUploadCSV = (e) => {
    ...
    Papa.parse(file, {
      ...
      complete: async (result) => {
        ...
        result.data.forEach((row) => {
          if (!grouped[row.userId]) {
            grouped[row.userId] = { user: row, modules: [], regions: [], batches: [] };
          }
          ...
          const accessRoles = row.accessRole ? row.accessRole.split(",") : [];
          // ✅ Changed class to batch
          const batchList = row.batch ? row.batch.split(",") : [];
          ...
          // ✅ Changed classes to batches
          grouped[row.userId].batches = batchList;
        ...
        // create users
        for (let uid in grouped) {
          const { user, modules, regions, batches } = grouped[uid];
          try {
            const userRes = await createUser({
              userId: user.userId,
              name: user.name,
    ...
    <Container className="my-4">
      <Card className="p-4 shadow-lg rounded-3">
        ...
        <Form onSubmit={handleSubmit}>
          ...
          <h5>Batch Access</h5>
          <div className="mb-3">
            {["2024-26", "2025-27", "2026-28"].map((batch) => (
              <Form.Check
                inline
                key={batch}
                label={batch}
